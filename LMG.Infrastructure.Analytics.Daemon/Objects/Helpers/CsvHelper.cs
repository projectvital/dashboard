...
namespace LMG.Infrastructure.Analytics.Daemon.Objects.Helpers
{
    ...
    //*******************************
    //Code below was taken from: http://www.blackbeltcoder.com/Articles/files/reading-and-writing-csv-files-in-c
    //*******************************
    ...
    public class CsvFileReader : CsvFileCommon, IDisposable
    {
        ...
        {
            Reader = new StreamReader(stream, Encoding.Default);//rmenten 2017-04-27: set Encoding to Default (ANSI) for special characters in CSV
        ...
        public CsvFileReader(string path,
            EmptyLineBehavior emptyLineBehavior = EmptyLineBehavior.NoColumns)
        {
            Reader = new StreamReader(path, Encoding.Default);//rmenten 2017-04-27: set Encoding to Default (ANSI) for special characters in CSV
            EmptyLineBehavior = emptyLineBehavior;
        }
    ...
    public class CsvFileWriter : CsvFileCommon, IDisposable
    {
        ...
        public void Dispose()
        {
            //Writer.Flush();//rmenten 2017-03-14
            //Writer.Close();
            Writer.Dispose();
        }
    }
